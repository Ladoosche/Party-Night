    ...
    <div className="flex-1 flex flex-col overflow-y-auto scrollbar-hide px-5 py-5 pb-12 md:pb-8 relative">
      ...
      <div className="space-y-3">
        ...
        <button 
          ...
          className="w-full text-left flex items-center gap-4 p-4 bg-white border border-slate-200 rounded-xl hover:border-purple-500 hover:bg-slate-50 group transition-all shadow-sm"
        >
          <div className="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center text-2xl group-hover:bg-white transition-colors border border-purple-100">🍸</div>
          <div className="flex-1">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">{t('nhie-title')}</h3>
              <span className="text-[8px] font-bold text-purple-500 uppercase tracking-wider bg-purple-50 px-1.5 py-0.5 rounded">
                {t('min-players').replace('{0}', '2')}
              </span>
            </div>
            <p className="text-[10px] text-slate-400 leading-tight font-medium mt-0.5">{t('nhie-desc')}</p>
          </div>
          <div className="text-slate-300 group-hover:text-purple-500 transition-colors">›</div>
        </button>
      </div>
    </div>
  );
