...
export const Game421: React.FC<Game421Props> = ({ onBack }) => {
  const { players: allPlayers, language, setLanguage, t } = useAppContext();
  const players = allPlayers.filter(p => p.isActive !== false);
  const [editPlayersMode, setEditPlayersMode] = React.useState(false);
  ...
  if (players.length < 1 && !editPlayersMode) {
    return (
      <NotEnoughPlayers 
        minPlayers={1} 
        onBack={onBack} 
  ...
  return (
    <div className="flex-1 flex flex-col bg-white overflow-hidden relative">
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 mb-4">
        <button
          onClick={showRules ? () => setShowRules(false) : onBack}
          className="p-2 -ml-2 text-slate-400 hover:text-slate-600 transition-colors"
        >
          <ChevronLeft size={24} />
        </button>
        <h2 className="text-sm font-bold uppercase tracking-widest text-slate-900">{t("game-421-title")}</h2>
      ...
      <AnimatePresence mode="wait">
        {showRules ? (
          ...
          >
             <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-8 mt-4">
                <h3 className="text-xs font-black uppercase tracking-widest text-slate-900 mb-4">{t("game-421-rules-title")}</h3>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8">
              <div className="mb-4">
                <button 
                  ...
                  className="px-4 py-2 bg-slate-50 text-slate-400 text-[10px] font-bold uppercase tracking-widest rounded-full border border-slate-100 hover:bg-slate-100 transition-all"
                >
                  {t("rules")}
                </button>
              </div>
